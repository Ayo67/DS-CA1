...
var handler = async (event, context) => {
  try {
    ...
    const getResult = await ddbDocClient.send(new import_lib_dynamodb2.GetCommand(params));
    if (!getResult.Item) {
      return {
        statusCode: 404,
        headers: { "content-type": "application/json" },
    ...
    const updateResult = await ddbDocClient.send(new import_lib_dynamodb2.UpdateCommand(updateParams));
    return {
      statusCode: 200,
      headers: { "content-type": "application/json" },
